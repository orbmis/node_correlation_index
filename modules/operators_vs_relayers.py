...
def collate_operators_vs_relays(input_file='collated.json', json_output_file='operators_vs_relayers.json', csv_output_file='operators_vs_relayers.csv'):
    ...
    with open(csv_output_file, mode='w', newline='') as csv_file:
        ...
        # Writing header
        header = ["Network Penetration", *next(iter(result_dict.values()))["relayers"].keys()]
...
def calculate_average_relayer_percentage_by_decile(json_data, num_deciles=10, potential_relays=None):
    if potential_relays is None:
        potential_relays = ["manifold","no_mev_boost","agnostic","aestus","bloxroute_regulated","edennetwork","ultra_sound_money","bloxroute_maxprofit","flashbots"]

    # Initialize decile data structure
    decile_data = {i: {relay: [] for relay in potential_relays} for i in range(num_deciles)}
    ...
    average_data = {i: {relay: sum(percentages) / len(percentages) if len(percentages) > 0 else 0 for relay, percentages in decile_data[i].items()} for i in range(num_deciles)}
    ...
    with open(csv_filename, mode='w', newline='') as csv_file:
        ...
        for decile, relay_percentages in average_data.items():
            row = [f"{decile * 10}-{(decile + 1) * 10 - 1}"] + [relay_percentages[relay] for relay in potential_relays]
            writer.writerow(row)
