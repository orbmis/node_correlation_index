...
def transform_data(input_file, output_file):
    ...
    for obj in data:
        ...
        if (
            ...
            pool_entry = next((tag for tag in obj['operatorTags'] if tag.get('idType') == 'pool'), None)
            ...
            operator_entry = next((entry for entry in transformed_data if entry['displayName'] == operator_name), None)
            ...
            operator_entry['totalValidatorCount'] += pool_object.get('validatorCount', 0)
            operator_entry['totalNetworkPenetration'] += pool_object.get('networkPenetration', 0)
